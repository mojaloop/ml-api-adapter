...
const processMessage = async (msg, span) => {
  ...
  if (actionLower === ENUM.Events.Event.Action.PREPARE && statusLower === ENUM.Events.EventStatus.SUCCESS.status) {
    const { url: callbackURLTo, childSpan } = await Participant.getEndpoint(to, ENUM.EndPoints.FspEndpointTypes.FSPIOP_CALLBACK_URL_TRANSFER_POST, id, span)
    const histTimerEnd = Metrics.getHistogram(
      `notification_handler_action_${actionLower.split('-').join('_')}_${statusLower}`,
      `sending action: ${actionLower} status: ${statusLower} to ${callbackURLTo}`,
      ['success']
    ).startTimer()
    const callbackToSpan = !!childSpan && childSpan.getChild(`${span.getContext().service}_${actionLower}_to`)
    !!callbackToSpan && callbackToSpan.setTags({ action: actionLower, from, to, method: ENUM.Http.RestMethods.POST, url: callbackURLTo })
    ...
    Logger.info(`[cid=${id}, fsp=${from}, source=${from}, dest=${to}] ~ ML-Notification::prepare::message - START`)
    await Callback.sendRequest(callbackURLTo, callbackHeaders, from, to, ENUM.Http.RestMethods.POST, payloadForCallback, ENUM.Http.ResponseTypes.JSON, span)
    !!callbackToSpan && callbackToSpan.finish()
    Logger.info(`[cid=${id}, fsp=${from}, source=${from}, dest=${to}] ~ ML-Notification::prepare::message - END`)
    histTimerEnd({ success: true })
    return true
  }

  if (actionLower === ENUM.Events.Event.Action.PREPARE && statusLower !== ENUM.Events.EventStatus.SUCCESS.status) {
    const { url: callbackURLTo, childSpan } = await Participant.getEndpoint(to, ENUM.EndPoints.FspEndpointTypes.FSPIOP_CALLBACK_URL_TRANSFER_ERROR, id, span)
    const histTimerEnd = Metrics.getHistogram(
      `notification_handler_action_${actionLower.split('-').join('_')}_${statusLower}`,
      `sending action: ${actionLower} status: ${statusLower} to ${callbackURLTo}`,
      ['success']
    ...
    const callbackToSpan = !!childSpan && childSpan.getChild(`${span.getContext().service}_${actionLower}_to`)
    !!callbackToSpan && callbackToSpan.setTags({ action: actionLower, from, to, method: ENUM.Http.RestMethods.PUT, url: callbackURLTo })
  ...
  if (actionLower === ENUM.Events.Event.Action.PREPARE_DUPLICATE && statusLower === ENUM.Events.EventStatus.SUCCESS.status) {
    const { url: callbackURLTo, childSpan } = await Participant.getEndpoint(to, ENUM.EndPoints.FspEndpointTypes.FSPIOP_CALLBACK_URL_TRANSFER_PUT, id, span)
    const histTimerEnd = Metrics.getHistogram(
      `notification_handler_action_${actionLower.split('-').join('_')}_${statusLower}`,
      `sending action: ${actionLower} status: ${statusLower} to ${callbackURLTo}`,
      ['success']
  ...
  if (actionLower === ENUM.Events.Event.Action.COMMIT && statusLower === ENUM.Events.EventStatus.SUCCESS.status) {
    const { url: callbackURLFrom, childSpan: childSpanFrom } = await Participant.getEndpoint(from, ENUM.EndPoints.FspEndpointTypes.FSPIOP_CALLBACK_URL_TRANSFER_PUT, id, span)
    const { url: callbackURLTo, childSpan: childSpanTo } = await Participant.getEndpoint(to, ENUM.EndPoints.FspEndpointTypes.FSPIOP_CALLBACK_URL_TRANSFER_PUT, id, span)
    const histTimerEnd = Metrics.getHistogram(
      `notification_handler_action_${actionLower.split('-').join('_')}_${statusLower}`,
      `sending action: ${actionLower} status: ${statusLower} to ${callbackURLTo}`,
      ['success']
    ).startTimer()
    const callbackToSpan = !!childSpanTo && childSpanTo.getChild(`${span.getContext().service}_${actionLower}_to`)
    !!callbackToSpan && callbackToSpan.setTags({ action: actionLower, from, to, method: ENUM.Http.RestMethods.PUT, url: callbackURLTo })
    ...
    Logger.debug(`Notification::processMessage - Callback.sendRequest(${callbackURLTo}, ${ENUM.Http.RestMethods.PUT}, ${JSON.stringify(callbackHeaders)}, ${payloadForCallback}, ${id}, ${from}, ${to})`)
    Logger.info(`[cid=${id}, fsp=${from}, source=${from}, dest=${to}] ~ ML-Notification::commit::message1 - START`)
    await Callback.sendRequest(callbackURLTo, callbackHeaders, from, to, ENUM.Http.RestMethods.PUT, payloadForCallback, ENUM.Http.ResponseTypes.JSON, span)
    !!callbackToSpan && callbackToSpan.finish()
    Logger.info(`[cid=${id}, fsp=${from}, source=${from}, dest=${to}] ~ ML-Notification::commit::message1 - END`)
    histTimerEnd({ success: true })
    // send an extra notification back to the original sender (if enabled in config)
    if (Config.SEND_TRANSFER_CONFIRMATION_TO_PAYEE) {
      const histTimerEndFrom = Metrics.getHistogram(
        `notification_handler_action_${actionLower.split('-').join('_')}_${statusLower}`,
        `sending action: ${actionLower} status: ${statusLower} from ${callbackURLFrom}`,
        ['success']
      ).startTimer()
      ...
      const callbackFromSpan = !!childSpanFrom && childSpanFrom.getChild(`${span.getContext().service}_${actionLower}_from`)
      !!callbackFromSpan && callbackFromSpan.setTags({ action: actionLower, from, to, method: ENUM.Http.RestMethods.PUT, url: callbackURLFrom })
      ...
      Logger.info(`[cid=${id}, fsp=${from}, source=${from}, dest=${to}] ~ ML-Notification::commit::message2 - START`)
      await Callback.sendRequest(callbackURLFrom, callbackHeaders, ENUM.Http.Headers.FSPIOP.SWITCH.value, from, ENUM.Http.RestMethods.PUT, payloadForCallback)
      !!callbackFromSpan && callbackToSpan.finish()
      Logger.info(`[cid=${id}, fsp=${from}, source=${from}, dest=${to}] ~ ML-Notification::commit::message2 - END`)
      Logger.info(`[cid=${id}, fsp=${from}, source=${from}, dest=${to}] ~ ML-Notification::commit::processMessage - END`)
      histTimerEndFrom({ success: true })
      return true
    } else {
  ...
  if (actionLower === ENUM.Events.Event.Action.COMMIT && statusLower !== ENUM.Events.EventStatus.SUCCESS.status) {
    const { url: callbackURLTo, childSpan: childSpanTo } = await Participant.getEndpoint(to, ENUM.EndPoints.FspEndpointTypes.FSPIOP_CALLBACK_URL_TRANSFER_ERROR, id, span)
    const histTimerEnd = Metrics.getHistogram(
      `notification_handler_action_${actionLower.split('-').join('_')}_${statusLower}`,
      `sending action: ${actionLower} status: ${statusLower} to ${callbackURLTo}`,
      ['success']
  ...
  if (actionLower === ENUM.Events.Event.Action.ABORT) {
    const { url: callbackURLFrom, childSpan: childSpanFrom } = await Participant.getEndpoint(from, ENUM.EndPoints.FspEndpointTypes.FSPIOP_CALLBACK_URL_TRANSFER_ERROR, id, span)
    const { url: callbackURLTo, childSpan: childSpanTo } = await Participant.getEndpoint(to, ENUM.EndPoints.FspEndpointTypes.FSPIOP_CALLBACK_URL_TRANSFER_ERROR, id, span)
    const histTimerEnd = Metrics.getHistogram(
      `notification_handler_action_${actionLower.split('-').join('_')}_${statusLower}`,
      `sending action: ${actionLower} status: ${statusLower} to ${callbackURLTo}`,
  ...
  if (actionLower === ENUM.Events.Event.Action.TIMEOUT_RECEIVED) {
    ...
    !!callbackToSpan && callbackToSpan.setTags({ action: actionLower, from, to, method: ENUM.Http.RestMethods.PUT, url: callbackURLTo })
    callbackHeaders = createCallbackHeaders({ dfspId: to, transferId: id, headers: content.headers, httpMethod: ENUM.Http.RestMethods.PUT, endpointTemplate: ENUM.EndPoints.FspEndpointTemplates.TRANSFERS_PUT_ERROR })
    Logger.debug(`Notification::processMessage - Callback.sendRequest(${callbackURLTo}, ${ENUM.Http.RestMethods.PUT}, ${JSON.stringify(callbackHeaders)}, ${payloadForCallback}, ${id}, ${to}, ${from})`)
    await Callback.sendRequest(callbackURLTo, callbackHeaders, from, to, ENUM.Http.RestMethods.PUT, payloadForCallback, ENUM.Http.ResponseTypes.JSON, span)
    !!callbackToSpan && callbackToSpan.finish()
    histTimerEnd({ success: true })
    return true
