  ...
  if ([Action.ABORT, Action.FX_ABORT].includes(action)) {
    ...
    const [, response] = await Promise.all([
      (async function notifyDestination () {
        ...
        const originalHeaders = { ...content.headers }
        if (action === Action.FX_ABORT && !isOriginalId) {
          // In the case where the Fx ABORT message is generated by the switch off of a transfer abort request from DFSP,
          // we need to modify the headers to have the correct content-type, source, and signature.
          fxAbortFromSwitch = true

          // set content type header to fxTransfer if not already set
          if (!originalHeaders['content-type'].includes(Enum.Http.HeaderResources.FX_TRANSFERS)) {
            originalHeaders['content-type'] = makeAcceptContentTypeHeader(Enum.Http.HeaderResources.FX_TRANSFERS, Config.PROTOCOL_VERSIONS.CONTENT.DEFAULT, Config.API_TYPE)
          }
        }
        const fxAbortJwsSigner = fxAbortFromSwitch ? jwsSigner : null
        headers = createCallbackHeaders({ dfspId: destination, transferId: id, headers: originalHeaders, httpMethod: PUT, endpointTemplate }, fxAbortFromSwitch)

        // forward the abort to the destination
        logger.debug(`Notification::processMessage - Callback.sendRequest({ ${callbackURLTo}, ${PUT}, ${JSON.stringify(headers)}, ${payload}, ${id}, ${source}, ${destination} ${hubNameRegex} })`)
        await Callback.sendRequest({ apiType: API_TYPE, url: callbackURLTo, headers, source, destination, method: PUT, payload, responseType, span, jwsSigner: fxAbortJwsSigner, protocolVersions, hubNameRegex })
      })(),
      (async function notifySource () {
        // send an extra notification back to the original sender (if enabled in config) and ignore this for on-us transfers
        if (sendToSource) {
          ...
          headers = createCallbackHeaders({ dfspId: source, transferId: id, headers: originalHeaders, httpMethod: PUT, endpointTemplate }, fxAbortFromSwitch)
          return await Callback.sendRequest({ apiType: API_TYPE, url: callbackURLFrom, headers, source: Config.HUB_NAME, destination: source, method: PUT, payload, responseType, span, jwsSigner, protocolVersions, hubNameRegex })
        } else {
