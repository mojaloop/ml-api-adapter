...
const prepare = async (headers, message) => {
  ...
  try {
    ...
    const topicConfig = Utility.createGeneralTopicConf(TRANSFER, PREPARE, 0, null, message.transferId)
...
const fulfil = async (id, headers, message) => {
  ...
  try {
    ...
    // }
    const topicConfig = Utility.createGeneralTopicConf(TRANSFER, FULFIL, 0, null, id)
