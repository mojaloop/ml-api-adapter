...
Test('Notification Service tests', async notificationTest => {
  ...
  await notificationTest.test('processMessage should', async processMessageTest => {
    ...
    await processMessageTest.test('process the fx abort message received from kafka and send out fx transfer put error callback', async test => {
      // This test covers processing of original fxabort message received from fxp via kafka, as opposed to the fx-abort message generated by the switch
      // as signified by the isOriginalId flag in the context object
      const uuid = Uuid()
      const payerFsp = 'payerFsp'
      const fxp = 'fxp1'
      const msg = {
    ...
    await processMessageTest.test('process the fx abort message received from kafka, modify headers and send out a fx transfer put callback', async test => {
      // This test covers the scenario where the switch generates the fx abort message off of a transfer abort request from a DFSP (isOriginalId = false)
      // The expectation is that the message is properly generated with fspiop-source being the switch, content-type being application/vnd.interoperability.fxTransfers+json;version=1.0
