...
Test('ParticipantEndpoint Service Test', endpointTest => {
  ...
  endpointTest.test('getEndpoint should', async (getEndpointTest) => {
    getEndpointTest.test('return the endpoint', async (test) => {
      ...
      const expected = { url: `http://localhost:1080/transfers/${transferId}`, childSpan: span }
      Facade.getEndpoint.withArgs(Config.ENDPOINT_SOURCE_URL, fsp, endpointType, { transferId }).returns(expected.url)

      try {
        const result = await Service.getEndpoint(fsp, endpointType, transferId, span)
        test.equal(result.url, expected.url, 'The results match')
        test.end()
      } catch (err) {
    ...
    getEndpointTest.test('return the endpoint when transferId is null', async (test) => {
      ...
      const expected = { url: 'http://localhost:1080/transfers', childSpan: span }
      Facade.getEndpoint.withArgs(Config.ENDPOINT_SOURCE_URL, fsp, endpointType).returns(expected.url)
