...
Test('transfer handler', handlerTest => {
  ...
  handlerTest.test('create should', createTransferTest => {
    createTransferTest.test('reply with status code 202 if message is sent successfully to kafka', test => {
      ...
      TransferService.prepare.returns(P.resolve({ payload }))
    ...
    createTransferTest.test('return error if transfer create throws', test => {
      const payload = {
      ...
      Handler.create(createRequest(payload), reply)
    })
