...
const create = (transfer) => {
  return Base.postApi(`/transfers`, transfer)
}
...
Test('POST /transfers', postTest => {
  postTest.test('should prepare a transfer', test => {
    ...
    const transfer = Fixtures.buildTransfer(transferId, 
      Fixtures.buildDebitOrCredit(Base.account1Name, amount), 
      Fixtures.buildDebitOrCredit(Base.account2Name, amount, memo))
  ...
  // postTest.test('should prepare and execute unconditional transfer', test => {
  //   const transferId = Fixtures.generateTransferId()
  //   const memo = { interledger: 'blah', path: 'blah' }
  ...
  // postTest.test('should not throw error on optimistic transfer with repeat id', test => {
  //   const transferId = Fixtures.generateTransferId()
  ...
  // postTest.test('should return transfer details when preparing existing transfer', test => {
  //   const transferId = Fixtures.generateTransferId()
  ...
  // postTest.test('should return error when preparing existing transfer with changed properties', test => {
  ...
  // postTest.test('return error when preparing fulfilled transfer', test => {
  //   let transferId = Fixtures.generateTransferId()
  //   let transfer = Fixtures.buildTransfer(transferId, Fixtures.buildDebitOrCredit(Base.account1Name, amount), Fixtures.buildDebitOrCredit(Base.account2Name, amount))
  ...
  // postTest.test('return error when preparing transfer with expired date', test => {
  //   const transferId = Fixtures.generateTransferId()
  ...
  // postTest.test('return error when preparing transfer with ledger account as sender', test => {
  //   const transferId = Fixtures.generateTransferId()
  ...
  // postTest.test('return error when preparing transfer with ledger account as receiver', test => {
  //   const transferId = Fixtures.generateTransferId()
  //   Config.LEDGER_ACCOUNT_NAME = 'LedgerAccountName'
